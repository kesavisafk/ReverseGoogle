...
## 2. Core Reverse Search Features

### 2.1 Intent Detection + Opposite Mapping
...
### 2.4 "I'm Feeling Confused" Curated Query Set
...
### 5.5 Submitted Screen + Timed Blue Screen
Success view shows:
...
- `reversegoogle.html` -> Main app UI and logic
- `search-results-generator.js` -> Opposite intent result generation
- `reverse-query.js` -> Query transformation utilities
- `opposites.js` -> Fallback opposite mapping
- `opposites.json` -> Externalized mapping data
- `impossible-form.html` -> Linked impossible form flow
- `api/unsplash-images.js` -> Unsplash serverless proxy
- `api/logo.js` -> Logo proxy
- `vercel.json` -> Vercel config

## 9. Behavioral Test Cases (Expected)
